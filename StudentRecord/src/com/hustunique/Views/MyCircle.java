...
public class MyCircle extends View{
    private static int DIAMETER = 180; //Diameter英文为直径，该常量表示小圆直径的dp值
    ...
    private int mWidth=200;// 这个值表示圆环的宽度的2倍（大圆直径-小圆直径）
    
    private final int defaultColor=Color.rgb(216,143,5);
    private int mbgcolor=Color.rgb(237,194,115);//进度条背景颜色
    private int processColor=Color.RED; //进度条进度颜色
    ...
    protected void onDraw(Canvas canvas) {
       ...
       /* if (mProgress < 360) {
        ...
        if (mProgress < 360) {
            ...
            canvas.drawArc(mRect, 140,260, false, paint);
        }
        {
            float degree = (float) (260.0f * mProgress);
            paint.setAlpha(0xff);
            ...
            //useCenter
            //paint表示要使用的画笔；
            canvas.drawArc(mRect, 140, degree, false, paint);
        }
    }
